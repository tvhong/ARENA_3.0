...
def unbroadcast(broadcasted: Arr, original: Arr) -> Arr:
    ...
    # Remove the leading dimensions of `broadcasted`
    leading_dims = len(broadcasted.shape) - len(original.shape)
    broadcasted = np.sum(broadcasted, axis=tuple(range(leading_dims)), keepdims=False)

    # Remove the middle dimensions of `broadcasted`
    dims_to_sum = tuple([
        i for i in range(len(broadcasted.shape)) if original.shape[i] == 1 and broadcasted.shape[i] != 1
    ])
    broadcasted = np.sum(broadcasted, axis=dims_to_sum, keepdims=True)
...
class BackwardFuncLookup:
    ...
    def get_back_func(self, forward_fn: Callable, arg_position: int) -> Callable:
        return self._back_funcs.get((forward_fn, arg_position))
...
def log_forward(x: Tensor) -> Tensor:
    ...
    out = Tensor(np.log(x.array), requires_grad=requires_grad)
    if requires_grad:
        out.recipe = Recipe(
            func=np.log,
            args=(x.array,),
...
def multiply_forward(a: Tensor | int, b: Tensor | int) -> Tensor:
    ...
    parents_requires_grad = (
        (isinstance(a, Tensor) and a.requires_grad)or
        (isinstance(b, Tensor) and b.requires_grad)
    )
    requires_grad = grad_tracking_enabled and parents_requires_grad
    ...
    out = Tensor(np.multiply(arg_a, arg_b), requires_grad=requires_grad)
    if requires_grad:
        parents = {}
...
def wrap_forward_fn(numpy_func: Callable, is_differentiable=True) -> Callable:
    ...
    def tensor_func(*args: Any, **kwargs: Any) -> Tensor:
        ...
        requires_grad = grad_tracking_enabled and is_differentiable and (any(isinstance(a, Tensor) and a.requires_grad for a in args))

        out = Tensor(numpy_func(*arg_arrays, **kwargs), requires_grad=requires_grad)
        if requires_grad:
    ...
    while frontier:
        ...
        for child in get_children(current):
            ...
            if in_degree[child] == 0:
                frontier.append(child)
            if in_degree[child] < 0:
                # If in-degree is negative, we have a cycle
...
def backprop(end_node: Tensor, end_grad: Tensor | None = None) -> None:
    ...
    def update_grad(tensor: Tensor, grad_arr: Arr) -> None:
        should_store_grad = tensor.requires_grad and tensor.is_leaf

        if tensor not in grads:
            grads[tensor] = grad_arr

            if should_store_grad:
                tensor.grad = Tensor(grad_arr, requires_grad=False)
        else:
            # If the tensor is already in grads, accumulate the gradient
            grads[tensor] += grad_arr

            if should_store_grad:
                tensor.grad.array += grad_arr
    ...
    for node in nodes:
        ...
        grad_out = grads[node] # Gradient must exist, otherwise the graph is disconnected
        out = node.array
        #print(f"{grad_out=}, {out=}")

        for i, parent in node.recipe.parents.items():
            back_func = BACK_FUNCS.get_back_func(node.recipe.func, i)
            if back_func is None:
                raise ValueError(
                    f"No backward function found for {node.recipe.func.__name__} at argument index {i}. If this argument is not a Tensor, it shouldn't be in recipe.parents."
                )

            parent_grad = back_func(grad_out, out, *node.recipe.args, **node.recipe.kwargs)

            update_grad(parent, parent_grad)

    # print(f"Final grads: {[grad for grad in grads.values()]}")
...
def sum_back(grad_out: Arr, out: Arr, x: Arr, dim=None, keepdim=False):
    ...
    # Dim is None means sum over all dimensions
    if dim is None:
        dim = tuple(range(len(x.shape)))

    # Add dimensions that were collapsed by the sum operation
    if not keepdim:
        grad_out = np.expand_dims(grad_out, axis=dim)

    # Broadcast along the dims that were summed over
...
def maximum_back0(grad_out: Arr, out: Arr, x: Arr, y: Arr):
    ...
    result = np.where(x > y, grad_out, np.where(np.isclose(x, y), grad_out / 2, 0))
    return unbroadcast(result, x)

def maximum_back1(grad_out: Arr, out: Arr, x: Arr, y: Arr):
    """Backwards function for max(x, y) wrt y."""
    result = np.where(y > x, grad_out, np.where(np.isclose(x, y), grad_out / 2, 0))
    return unbroadcast(result, y)
...
def relu(x: Tensor) -> Tensor:
    ...
    return maximum(x, Tensor(0.0, requires_grad=x.requires_grad))
...
class Linear(Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias=True):
        ...
        sf = 1 / np.sqrt(in_features)  # Scaling factor for weight initialization
        self.weight = Parameter(Tensor(np.random.uniform(-sf, sf, (out_features, in_features))))
        if bias:
            self.bias = Parameter(Tensor(np.random.uniform(-sf, sf, (out_features,))))
...
def cross_entropy(logits: Tensor, true_labels: Tensor) -> Tensor:
    ...
    log_probs = logits - sum_exp.log()

    # Select the log probabilities corresponding to the true labels
    batch_size = logits.shape[0]
    selected_log_probs = log_probs[arange(0, batch_size), true_labels]
